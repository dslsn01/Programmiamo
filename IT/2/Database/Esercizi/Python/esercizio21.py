...
try:
	...
	if title: params["title"] = "%{}%".format(title)

	year = input("Anno: ")

	if year: params["year"] = "%{}%".format(year)

	director = input("Regista: ")

	if director: params["director"] = "%{}%".format(director)

	star = input("Attore principale: ")

	if star: params["star"] = "%{}%".format(star)

	if len(params) != 0:
		i = 0
		for k in list(params.keys()):
			clause = (" WHERE {}" if i == 0 else " AND {}").format(k)
			clause += " LIKE CAST(? AS VARCHAR)" if k == "year" else " LIKE ?"
			query += clause
			i += 1
