...
eseguire con: java -cp .:sqlitejdbc-v<nnn>.jar Esercizio21
*/
		...
		try {
			...
			if (params.size() != 0) {
				...
				for (Entry<String, String> item : params.entrySet()) {
					...
					String clause = ((i == 0) ? " WHERE " : " AND ") + k;
					clause += (k.equals("year")) ? " LIKE CAST(? AS VARCHAR)" : " LIKE ?";
					query += clause;
					i++;
				}
			...
			if (!res.isBeforeFirst()) {
				System.err.println("Film non disponibile in videoteca!");
				pStmt.close();
				conn.close();
			}
