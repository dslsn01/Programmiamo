...
class ATMCashWtdrException : public logic_error {
	using logic_error::logic_error;
};

class ATMCashDepoException : public logic_error {
	using logic_error::logic_error;
};
		...
		try {
			if (oper == "1") {
				...
				if (this->balance - cash >= -500) {
					...
					throw ATMCashWtdrException("Conto incapiente!");
				}
			}
			else if (oper == "3") {
				this->enterPin();
				...
				if (cash <= 10000) {
					...
					throw ATMCashDepoException("Importo (" + to_string(cash) + " euro) superiore al massimo consentito!");
				}
			}
			else if (oper == "4") {
